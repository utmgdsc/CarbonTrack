...
export const TransportationAPI = {
  getTransportation: async (transportationID: ObjectId) => {
    try {
      const res = await FLASK_HTTPS.get(
        routeName + '/transportation/' + transportationID.toHexString()
      );
  ...
  getTransportationEntriesForUserUsingDataRange: async (start: Date, end: Date) => {
    try {
      const res = await FLASK_HTTPS.post(
        routeName + '/get_transportation_entries_for_user_using_data_range',
        {
          start,
          end,
        }
  ...
  ): Promise<undefined | TransportationEntry> => {
    try {
      const res = await FLASK_HTTPS.patch(
        routeName + '/transportation/' + transportation._id.toString(),
        {
          transportation,
        }
      );
  ...
  getTransportationRecommendationForToday: async (): Promise<
    undefined | TransportationRecommendation
  > => {
    try {
      const res = await FLASK_HTTPS.get(routeName + '/get_transportation_recommendation_for_today');
      return res.data.transportation_recommendation as TransportationRecommendation;
    } catch (error) {
      console.error('Error fetching transportation recommendation from Flask BE: ', error);
      console.error('Temp tip: have you started the backend?: ');
      return undefined;
    }
  },
