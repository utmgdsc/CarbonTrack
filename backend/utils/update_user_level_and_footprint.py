...
def update_user_level_and_footprint(user_id: ObjectId) -> None:
    ...
    transportation_entries_dicts: list[dict] = list(
        CarbonTrackDB.transportation_coll.find({"user_id": user.oid})
    )
    transportation_entries: list[TransportationEntry] = [
        TransportationEntry.from_json(entry) for entry in transportation_entries_dicts
    ]
    transportation_entries = sorted(
        transportation_entries, key=lambda entry: entry.date
    )
    transportation_entries_overall = [
        entry
        for entry in transportation_entries
        if entry.calculate_carbon_emissions() != 0
    ]
    transportation_entries_yearly = [
        entry
        for entry in transportation_entries
        if entry.calculate_carbon_emissions() != 0 and _is_within_one_year(entry.date)
    ]
    transportation_entries_monthly = [
        entry
        for entry in transportation_entries
    ...
    if transportation_entries_overall.__len__() != 0:
        ...
        for entry in transportation_entries_overall[1:]:
            if (
                last_entry.calculate_carbon_emissions()
                > entry.calculate_carbon_emissions()
            ):
    ...
    overall_transportation_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in transportation_entries_overall]
    )
    yearly_transportation_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in transportation_entries_yearly]
    )
    monthly_transportation_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in transportation_entries_monthly]
    )

    # Get Filtered and Sorted food Entries
    food_entries_dicts: list[dict] = list(
        CarbonTrackDB.food_coll.find({"user_id": user.oid})
    )
    food_entries: list[FoodEntry] = [
        FoodEntry.from_json(entry) for entry in food_entries_dicts
    ]
    food_entries = sorted(food_entries, key=lambda entry: entry.date)
    food_entries_overall = [
        entry for entry in food_entries if entry.calculate_carbon_emissions() != 0
    ]
    food_entries_yearly = [
        entry
    ...
    overall_food_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in food_entries_overall]
    )
    yearly_food_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in food_entries_yearly]
    )
    monthly_food_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in food_entries_monthly]
    )

    # Get Filtered and Sorted energy Entries
    energy_entries_dicts: list[dict] = list(
        CarbonTrackDB.energy_coll.find({"user_id": user.oid})
    )
    energy_entries: list[EnergyEntry] = [
        EnergyEntry.from_json(entry) for entry in energy_entries_dicts
    ]
    energy_entries = sorted(energy_entries, key=lambda entry: entry.date)
    energy_entries_overall = [
        entry for entry in energy_entries if entry.calculate_carbon_emissions() != 0
    ]
    energy_entries_yearly = [
        entry
        for entry in energy_entries
        if entry.calculate_carbon_emissions() != 0 and _is_within_one_year(entry.date)
    ]
    energy_entries_monthly = [
        entry
        for entry in energy_entries
        if entry.calculate_carbon_emissions() != 0 and _is_within_one_month(entry.date)
    ]
    ...
    overall_energy_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in energy_entries_overall]
    )
    yearly_energy_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in energy_entries_yearly]
    )
    monthly_energy_entries_emissions = sum(
        [entry.calculate_carbon_emissions() for entry in energy_entries_monthly]
    )
    ...
    # Combine Emissions
    overall_carbon_emissions = (
        overall_transportation_entries_emissions
        + overall_food_entries_emissions
        + overall_energy_entries_emissions
    )
    yearly_carbon_emissions = (
        yearly_transportation_entries_emissions
        + yearly_food_entries_emissions
        + yearly_energy_entries_emissions
    )
    monthly_carbon_emissions = (
        monthly_transportation_entries_emissions
        + monthly_food_entries_emissions
        + monthly_energy_entries_emissions
    )
    ...
    user.badges = badges
    CarbonTrackDB.users_coll.update_one({"_id": user.oid}, {"$set": user.to_json()})
    # user_d = user.to_json()
